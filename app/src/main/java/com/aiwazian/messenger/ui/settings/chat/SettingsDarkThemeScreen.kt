...
private data class ThemeItem(
    val name: String, val theme: ThemeOption
)
...
private fun Content() {
    ...
    Scaffold(
        ...
        Column(
            ...
            val selectedOption = themeService.currentTheme.collectAsState().value
            val themes = listOf(
                ThemeItem("Как в системе", ThemeOption.SYSTEM),
                ThemeItem("Включена", ThemeOption.DARK),
                ThemeItem("Отключена", ThemeOption.LIGHT)
            )
...
private fun TopBar() {
    ...
    PageTopBar(
        title = { Text(stringResource(R.string.dark_theme)) }, navigationIcon = {
            IconButton(onClick = {
